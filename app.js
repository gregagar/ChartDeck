...
const els = {
  ...
  findOnlineButton: $("findOnlineButton"),
  ...
  onlineImportForm: $("onlineImportForm"),
  onlineTitleInput: $("onlineTitleInput"),
  onlineArtistInput: $("onlineArtistInput"),
  onlineKeyInput: $("onlineKeyInput"),
  onlineTempoInput: $("onlineTempoInput"),
  onlineTagsInput: $("onlineTagsInput"),
  onlineChartInput: $("onlineChartInput"),
  onlineUrlInput: $("onlineUrlInput"),
  fetchOnlineUrlButton: $("fetchOnlineUrlButton"),
  searchWebButton: $("searchWebButton"),
  searchChordProButton: $("searchChordProButton"),
  pasteClipboardButton: $("pasteClipboardButton"),
  convertOnlineChartToggle: $("convertOnlineChartToggle"),
  onlineImportStatus: $("onlineImportStatus"),
...
function openOnlineImporter(prefillTitle = "", prefillArtist = "") {
  els.onlineImportForm.reset();
  els.convertOnlineChartToggle.checked = true;
  els.onlineTitleInput.value = prefillTitle;
  els.onlineArtistInput.value = prefillArtist;
  els.onlineImportStatus.textContent = "ChordPro title, artist, key, tempo, capo and tag directives will be recognised automatically.";
  openDialog(els.onlineImportDialog);
  setTimeout(() => (prefillTitle ? els.onlineArtistInput : els.onlineTitleInput).focus(), 60);
}

function onlineSearchQuery(mode = "general") {
  const title = els.onlineTitleInput.value.trim();
  const artist = els.onlineArtistInput.value.trim();
  if (!title && !artist) {
    showToast("Add a title or artist first");
    els.onlineTitleInput.focus();
    return;
  }
  const base = [title, artist].filter(Boolean).join(" ");
  const query = mode === "chordpro"
    ? `${base} ChordPro (filetype:cho OR filetype:chopro OR filetype:pro)`
    : `${base} chords lyrics`;
  window.open(`https://www.google.com/search?q=${encodeURIComponent(query)}`, "_blank", "noopener,noreferrer");
  els.onlineImportStatus.textContent = "Search opened in a new tab. Copy a chart you are permitted to use, return here, then tap Paste from Clipboard.";
}

function parseChordProMetadata(rawText = "") {
  const metadata = { title: "", artist: "", key: "", tempo: "", capo: "", tags: [] };
  const keptLines = [];
  String(rawText).replaceAll("\r\n", "\n").split("\n").forEach(line => {
    const match = line.trim().match(/^\{\s*([^:}]+)\s*:\s*(.*?)\s*\}$/);
    if (!match) { keptLines.push(line); return; }
    const directive = match[1].toLowerCase().replaceAll("-", "_").trim();
    const value = match[2].trim();
    if (["title", "t"].includes(directive)) metadata.title ||= value;
    else if (["artist", "subtitle", "st"].includes(directive)) metadata.artist ||= value;
    else if (["key", "key_actual"].includes(directive)) metadata.key ||= value;
    else if (["tempo", "bpm"].includes(directive)) metadata.tempo ||= value.match(/\d+/)?.[0] || "";
    else if (directive === "capo") metadata.capo ||= value.match(/\d+/)?.[0] || "";
    else if (["tag", "tags"].includes(directive)) metadata.tags.push(...value.split(/[,;|]/).map(item => item.trim()).filter(Boolean));
    else keptLines.push(line);
  });
  metadata.tags = [...new Set(metadata.tags)];
  return { metadata, chart: keptLines.join("\n").trim() };
}

function isChordLine(line = "") {
  const trimmed = line.trim();
  if (!trimmed || trimmed.length > 120 || /[a-z]{3,}/.test(trimmed)) return false;
  const tokens = trimmed.split(/\s+/).filter(Boolean);
  if (!tokens.length || tokens.length > 20) return false;
  return tokens.every(token => {
    const clean = token.replace(/^[|:,(]+|[|:),.]+$/g, "");
    return !clean || looksLikeChordToken(clean) || /^(?:x\d+|repeat|N\.C\.)$/i.test(clean);
  });
}
...
function mergeChordLineWithLyrics(chordLine, lyricLine) {
  ...
  while ((match = tokenRegex.exec(chordLine)) !== null) {
    ...
    if (looksLikeChordToken(clean) || /^N\.?C\.?$/i.test(clean)) placements.push({ index: match.index, chord: clean });
  }
  let result = String(lyricLine || "");
  placements.sort((a, b) => b.index - a.index).forEach(({ index, chord }) => {
    const safeIndex = Math.max(0, Math.min(result.length, index));
    result = `${result.slice(0, safeIndex)}[${chord}]${result.slice(safeIndex)}`;
  });
  return result;
}

function convertPlainChartToChordPro(rawText = "") {
  const source = String(rawText).replaceAll("\r\n", "\n");
  if (/\[[A-G](?:#|b)?/.test(source)) return { chart: source.trim(), convertedPairs: 0 };
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (isChordLine(line) && next !== undefined && next.trim() && !isChordLine(next)) {
      output.push(mergeChordLineWithLyrics(line, next));
      convertedPairs += 1;
      index += 1;
      continue;
    }
    const trimmed = line.trim();
    if (trimmed && trimmed.length <= 30 && /^(?:verse|chorus|bridge|intro|outro|pre[- ]?chorus|instrumental|solo|middle 8|ending)(?:\s*\d+)?\s*:?$/i.test(trimmed)) {
      output.push(`[${trimmed.replace(/:$/, "")}]`);
    } else output.push(line);
  }
  return { chart: output.join("\n").trim(), convertedPairs };
}

function inferKeyFromChart(chart = "") {
  const match = String(chart).match(/\[([A-G](?:#|b)?(?:m|maj|min)?)/);
  return match?.[1] || "";
}


async function fetchOnlineChartUrl() {
  const rawUrl = els.onlineUrlInput.value.trim();
  if (!rawUrl) { showToast("Paste a direct chart URL first"); els.onlineUrlInput.focus(); return; }
  let url;
  try {
    url = new URL(rawUrl);
    if (!["http:", "https:"].includes(url.protocol)) throw new Error("Unsupported protocol");
  } catch (error) {
    showToast("That does not look like a valid web address");
    return;
  }
  els.fetchOnlineUrlButton.disabled = true;
  els.fetchOnlineUrlButton.textContent = "Importing…";
  try {
    const response = await fetch(url.href, { mode: "cors", credentials: "omit" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const text = await response.text();
    if (!text.trim()) throw new Error("Empty response");
    els.onlineChartInput.value = text;
    const { metadata } = parseChordProMetadata(text);
    if (!els.onlineTitleInput.value) els.onlineTitleInput.value = metadata.title;
    if (!els.onlineArtistInput.value) els.onlineArtistInput.value = metadata.artist;
    if (!els.onlineKeyInput.value) els.onlineKeyInput.value = metadata.key;
    if (!els.onlineTempoInput.value) els.onlineTempoInput.value = metadata.tempo;
    if (!els.onlineTagsInput.value && metadata.tags.length) els.onlineTagsInput.value = metadata.tags.join(", ");
    els.onlineImportStatus.textContent = `Imported ${text.length.toLocaleString()} characters from the direct URL. Review it before continuing.`;
    showToast("Chart imported from URL");
  } catch (error) {
    console.warn("Direct chart URL import failed", error);
    els.onlineImportStatus.textContent = "That website blocked direct browser import. Open it normally, copy the chart text, and use Paste from Clipboard instead.";
    showToast("Direct import was blocked by the website");
  } finally {
    els.fetchOnlineUrlButton.disabled = false;
    els.fetchOnlineUrlButton.textContent = "Import URL";
  }
}

async function pasteOnlineChartFromClipboard() {
  ...
  try {
    ...
    if (!els.onlineTempoInput.value) els.onlineTempoInput.value = metadata.tempo;
    if (!els.onlineTagsInput.value && metadata.tags.length) els.onlineTagsInput.value = metadata.tags.join(", ");
    els.onlineImportStatus.textContent = `Pasted ${text.length.toLocaleString()} characters. Review the chart, then continue to the Song Editor.`;
  } catch (error) {
    console.warn("Clipboard read failed", error);
    showToast("Allow clipboard access, or paste into the box manually");
  }
}

function moveOnlineImportToEditor() {
  const raw = els.onlineChartInput.value;
  if (!raw.trim()) { showToast("Paste a chart first"); els.onlineChartInput.focus(); return; }
  const parsed = parseChordProMetadata(raw);
  const converted = els.convertOnlineChartToggle.checked ? convertPlainChartToChordPro(parsed.chart) : { chart: parsed.chart, convertedPairs: 0 };
  const chart = converted.chart;
  const title = els.onlineTitleInput.value.trim() || parsed.metadata.title;
  const artist = els.onlineArtistInput.value.trim() || parsed.metadata.artist;
  const key = els.onlineKeyInput.value.trim() || parsed.metadata.key || inferKeyFromChart(chart);
  const tempo = els.onlineTempoInput.value || parsed.metadata.tempo;
  const tags = els.onlineTagsInput.value.trim() || parsed.metadata.tags.join(", ");
  closeDialog(els.onlineImportDialog);
  openSongEditor();
  els.songTitleInput.value = title;
  els.songArtistInput.value = artist;
  els.songKeyInput.value = key;
  els.songTempoInput.value = tempo;
  els.songTagsInput.value = tags;
  els.songCapoInput.value = parsed.metadata.capo || 0;
  els.songChartInput.value = chart;
  setTimeout(() => {
    if (!title) els.songTitleInput.focus();
    else if (!artist) els.songArtistInput.focus();
    else if (!key) els.songKeyInput.focus();
    else els.songChartInput.focus();
  }, 80);
  showToast(converted.convertedPairs ? `Converted ${converted.convertedPairs} chord line${converted.convertedPairs === 1 ? "" : "s"}` : "Chart imported for review");
}
...
els.findOnlineButton.addEventListener("click", () => openOnlineImporter());
...
els.onlineImportForm.addEventListener("submit", event => { event.preventDefault(); moveOnlineImportToEditor(); });
els.searchWebButton.addEventListener("click", () => onlineSearchQuery("general"));
els.searchChordProButton.addEventListener("click", () => onlineSearchQuery("chordpro"));
els.pasteClipboardButton.addEventListener("click", pasteOnlineChartFromClipboard);
els.fetchOnlineUrlButton.addEventListener("click", fetchOnlineChartUrl);
