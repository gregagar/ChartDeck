...
const els = {
  ...
  enharmonicButton: $("enharmonicButton"),
  enharmonicLabel: $("enharmonicLabel"),
...
function applySettings() {
  ...
  els.autoScrollToggle.checked = settings.autoScroll;
...
function renderChart() {
  ...
  const controls = [els.editSongButton, els.quickEditButton, els.favouriteButton, els.songMoreButton,
    ...
    els.capoButton, els.tempoButton, els.chartMoreButton];
...
function updateTransposeDialog() {
  ...
  els.enharmonicLabel.textContent = song.useFlats ? "Flats (♭) ›" : "Sharps (#) ›";
...
function showSongActions(songId) {
  const song = songs.find(candidate => candidate.id === songId);
  if (!song) return;
  const setlist = getActiveSetlist();
  const action = prompt(
    `Choose an action for “${song.title}”:\n\n1 — Open chart\n2 — Add to ${setlist?.name || "a setlist"}\n3 — ${song.favourite ? "Remove favourite" : "Add favourite"}\n4 — Edit\n5 — Delete`,
    "1"
  );
  if (action === "1") selectSong(songId, filteredSongs().map(item => item.id));
  if (action === "2") addSongToSetlist(songId);
  if (action === "3") toggleFavourite(songId);
  if (action === "4") openSongEditor(songId);
  if (action === "5") deleteSong(songId);
}
...
function startAutoScroll() {
  ...
  const step = time => {
    ...
    const elapsed = (time - lastAutoScrollTime) / 1000;
    lastAutoScrollTime = time;
    els.chartPaper.scrollTop += settings.autoScrollSpeed * elapsed;
...
function cycleCapo() {
  const song = getActiveSong();
  if (!song) return;
  song.capo = ((Number(song.capo) || 0) + 1) % 13;
  saveAll();
  renderChart();
  showToast(song.capo ? `Capo ${song.capo}` : "Capo off");
}
...
els.songTable.addEventListener("click", event => {
  ...
  if (menu) showSongActions(menu.dataset.songMenu);
});
...
els.songMoreButton.addEventListener("click", () => showSongActions(activeSongId));
...
els.enharmonicButton.addEventListener("click", () => {
  const song = getActiveSong();
  if (!song) return;
  song.useFlats = !song.useFlats;
  saveAll();
  renderChart();
});
...
els.backToLibraryButton.addEventListener("click", () => document.body.classList.remove("mobile-chart-open"));
els.capoButton.addEventListener("click", cycleCapo);
els.tempoButton.addEventListener("click", tapTempo);
els.tapTempoSettingsButton.addEventListener("click", tapTempo);
els.chartMoreButton.addEventListener("click", () => openDialog(els.themeDialog));
...
els.autoScrollToggle.addEventListener("change", event => {
  settings.autoScroll = event.target.checked;
  saveAll();
  applySettings();
